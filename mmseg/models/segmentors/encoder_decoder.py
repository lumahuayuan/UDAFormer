...
class EncoderDecoder(BaseSegmentor):
    ...
    def encode_decode(self, img, img_metas):
        ...
        out_scaled = resize(input=out, size=img.shape[2:], mode='bilinear', align_corners=self.align_corners)

        return out_scaled, out
    ...
    def forward_train(self,
        ...
        if isinstance(img, list):
            img_strong = img[1]
            losses['features_strong'] = self.extract_feat(img_strong)
            img = img[0]

        x = self.extract_feat(img)

        if return_feat:
            losses['features_week'] = x
